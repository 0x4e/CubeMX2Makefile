...
mcu_regex_to_DSP_dict = {
    'STM32(F|L)0': 'ARM_MATH_CM0',
    'STM32(F|L)1': 'ARM_MATH_CM3',
    'STM32(F|L)2': 'ARM_MATH_CM3',
    'STM32(F|L)3': 'ARM_MATH_CM4',
    'STM32(F|L)4': 'ARM_MATH_CM4',
    'STM32(F|L)7': 'ARM_MATH_CM7',
}

mcu_regex_to_OpenOCD_dict = {
    'STM32(F|L)0': 'target/stm32f0x.cfg',
    'STM32(F|L)1': 'target/stm32f1x.cfg',
    'STM32(F|L)2': 'target/stm32f2x.cfg',
    'STM32(F|L)3': 'target/stm32f3x.cfg',
    'STM32(F|L)4': 'target/stm32f4x.cfg',
    'STM32(F|L)7': 'target/stm32f7x.cfg',
}
...
def main():
    ...
    os.chdir("../CMSIS_DSP")
    DSP_template_file_path = os.path.join(os.getcwd(), 'DSPMakefile.tpl')
    try:
        with open(DSP_template_file_path, 'rb') as f:
            DSP_makefile_template = string.Template(f.read())
    except EnvironmentError as e:
        sys.stderr.write("Unable to read DSP template file: {}. Error: {}".format(DSP_template_file_path, str(e)))
        sys.exit(C2M_ERR_LOAD_TEMPLATE)
        ...
        for file in files:
            for s in ctx:
                ...
                if file.endswith(s['source_endswith']):
                    ...
                    #Do not add any files from path CMSIS/DSP
                    if relpath.startswith("Drivers/CMSIS/DSP/"):
                        break
                    s['source_subst'] += ' \\\n  '
    ...
    for mcu_regex_pattern, openOCD_MCU in mcu_regex_to_OpenOCD_dict.items():
        if re.match(mcu_regex_pattern, mcu_str):
            openOCD_subst = openOCD_MCU
    ...
    #Copy DSP Lib from:
    # /home/nick/ARM/Utilities/CMSIS_DSP/
    # to:
    # /PROJECT_FOLDER/Drivers/CMSIS/DSP/
    
    #Move to CMSIS DSP directory
    os.chdir("../CMSIS_DSP")
    
    source_dir = os.getcwd()
    destination_dir = proj_folder_path +"/Drivers/CMSIS/DSP/"
    
    from distutils.dir_util import copy_tree
    copy_tree(source_dir, destination_dir)
    
    #delete template makefile from destination dir
    os.chdir(destination_dir)
    os.remove("DSPMakefile.tpl")

    DSP_makefile_str = DSP_makefile_template.substitute(
        TARGET = proj_name,
        MCU = cflags_subst,
        LDMCU = ld_subst,
        AS_DEFS = DSP_c_defs_subst,
        C_DEFS = DSP_c_defs_subst,
        C_INCLUDES = c_set['inc_subst'],
        LDSCRIPT = ld_script_subst)
    
    DSP_makefile_path = os.path.join(proj_folder_path +"/Drivers/CMSIS/DSP/", 'Makefile')
    try:
        with open(DSP_makefile_path, 'wb') as f:
            f.write(DSP_makefile_str)
    except EnvironmentError as e:
        sys.stderr.write("Unable to write DSP Makefile: {}. Error: {}\n".format(DSP_makefile_path, str(e)))
        sys.exit(C2M_ERR_IO)

    sys.stdout.write("DSP Makefile created: {}\n".format(DSP_makefile_path))
